...
public partial class CommandWindow : Window
{
    ...
    internal CommandWindow(App? app)
    {
        ...
        for (int i = 0; i < Chapter.All.Length; i++)
        {
            ...
            stack.Children.Add(new TextBlock { Text = ch.Name, FontSize = 11, Margin = new Thickness(0, 5, 0, 0), HorizontalAlignment = HorizontalAlignment.Center });
            var button = new Button { Content = stack, Tag = index, Margin = new Thickness(4), Padding = new Thickness(3, 5, 3, 5), ToolTip = ch.Weapon };
    ...
    internal void Refresh()
    {
        ...
        ChapterName.Text = chapter.Name; ChapterEnglish.Text = chapter.English; WeaponName.Text = chapter.Weapon;
        CombatDetail.Text = CombatStyle.All[index].Detail;
        ...
        MenuNote.Text = app?.Preview == true ? "预览模式 · 未修改系统右键菜单" : app?.MenuStatus ?? "Windows 11：右键 → 显示更多选项";
        StatusText.Text = app?.Pet.Status ?? "等待命令"; ReportText.Text = app?.Pet.LastReport ?? "战士已就位。为了帝皇。";
