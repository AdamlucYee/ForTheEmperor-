![For The Emperor 主视觉](img/5.png)

# FOR THE EMPEROR · 桌面战士

**以帝皇之名，肃清你的桌面。**

Windows 桌宠 v2.2 中英双语版。WPF 透明窗口、八套像素写实装甲精灵、独立战团动作与动态粒子，以及“先攻击动画，再将桌面文件移入回收站”的真实执行链路。

[English guide](README.en.md)

## v2.2 中英双语更新

- 首次启动或从无语言设置的旧版升级时，显示简体中文／English 选择窗口；选择后保存，下次直接进入。
- 指挥面板的“语言 / Language”下拉框支持即时切换，无需重启，不重置战团、尺寸或任务。
- 主面板、桌宠与托盘菜单、文件右键命令、动作说明、状态报告、演练标记和应用自身错误提示均支持英语。Windows 自身的系统对话框仍由系统语言决定。
- 调整英文长文本布局；可用屏幕高度较小时，面板内容支持滚动。
- 延续自带运行时的 Windows x64 EXE。[下载 v2.2 对外便携包](release/ForTheEmperor-v2.2-win-x64.zip)：解压到单独文件夹，直接双击 EXE，无需另装 .NET。
...
双击根目录或 `dist` 中的 `ForTheEmperor.exe`，首次启动选择语言。v2.2 为自包含的单文件程序，需要 **64 位 Windows 10/11**，**无需另装 .NET 或 SDK**。角色素材和桌面运行时均包含在 EXE 内，无需复制 `bin`、`obj` 等构建目录。无需管理员权限，不设开机自启，程序运行不联网。现有未带 v2.2 名称的便携 ZIP 是旧版，不会随本次本地构建自动更新。

首次启动会在当前用户的临时目录解压内置的本机运行库，可能比后续启动稍慢。旧版 v0.2 的约 14 MB EXE 仍需另装 .NET 8 Windows Desktop Runtime；若看到“必须安装或更新 .NET”，请换用新版自包含 EXE。
...
`build.ps1` 默认生成本地自包含 EXE，并嵌入本机运行库、压缩内置程序集，不进行 WPF 不支持的裁剪。`-Test` 对实际发布的 EXE 运行自检，通过后再复制到根目录和 `dist`。添加 `-Package` 会调用 `package.ps1`，在 `release` 中生成独立便携文件夹、同名单层 ZIP 和 SHA-256 校验文件。已有验证过的构建也可直接执行 `./package.ps1` 打包。

`verify-language.ps1` 验证旧设置首次语言选择、英语保存、实时切换、Explorer 命令翻译、重启和菜单清理；使用临时设置并在结束时恢复原设置，不操作桌面文件。运行前先退出桌宠。

预览与调试支持 `--preview --language en`、`--preview --language zh-CN` 和 `--preview --choose-language`，不会保存设置或注册文件右键菜单。`--render-preview artifacts/en --language en` 可输出英文预览图。
